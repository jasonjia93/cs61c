...
public class Proj1{
	...
	private static double rate(DoublePair pair) {
		double a = pair.getDouble1();
		double s = pair.getDouble2();
		if (s > 0) {
			return (s * Math.pow(Math.log(s), 3))/a;
		} else {
			return 0;
		}
	}
        ...
        @Override
            ...
            throws IOException, InterruptedException {
                ...
                while (matcher.find()) {
					...
					pair.setDouble1(1.0);
					if ((word.toString()).equals(targetGram) == false) {
						for (int n: lst) {
							if (Math.abs(i-n) < d) {
								d = Math.abs(i-n);
							}
        ...
        @Override
            public void reduce(Text key, Iterable<DoublePair> values,
				...
				for (DoublePair pair: values) {
					asum += pair.getDouble1();
					ssum += pair.getDouble2();
				}
				context.write(key, new DoublePair(asum, ssum));
            }
    }
    ...
    public static class Reduce1 extends Reducer<Text, DoublePair, DoubleWritable, Text> {
        @Override
            public void reduce(Text key, Iterable<DoublePair> values,
				...
				DoubleWritable dw = new DoubleWritable(rate(new DoublePair(asum, ssum)));
